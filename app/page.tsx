...
export default function TypingApp() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      setLiderler(snapshot.docs.map(doc => doc.data() as Lider));
    });
    return () => unsubscribe();
  }, [appMode]);
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '800px', margin: '0 auto', textAlign: 'center', fontFamily: 'sans-serif', backgroundColor: '#f7fafc', minHeight: '100vh' }}>
        ...
        {appMode === 'shooter' ? (
          <div style={{ position: 'relative', width: '100%', height: '400px', backgroundColor: '#1a202c', borderRadius: '15px', overflow: 'hidden', border: '3px solid #2d3748', marginBottom: '20px' }}>
             {testEnded ? (
               ...
               enemies.map(en => (
                 <div key={en.id} style={{ position: 'absolute', top: en.y + '%', left: en.x + '%', background: 'white', padding: '5px 12px', borderRadius: '8px', fontWeight: 'bold', color: '#1a202c' }}>
                   {en.word}
                 </div>
               ))
             )}
      ...
      {testEnded && (
        <div style={{ marginTop: '20px', padding: '20px', background: '#fff', borderRadius: '15px', border: '2px solid #3182ce' }}>
          <h3>Nəticə: {appMode === 'shooter' ? `${score} Xal` : `${correct} wpm`}</h3>
          {!isSaved ? (
            <div>
              <input placeholder="Adınız" value={userName} onChange={(e) => setUserName(e.target.value)} style={{ padding: '10px', marginRight: '10px' }} />
              <button onClick={handleSaveRecord} style={{ padding: '10px 20px', background: '#48bb78', color: 'white', border: 'none', borderRadius: '5px' }}>Saxla</button>
            </div>
          ) : <p>✓ Qeyd edildi!</p>}
          <button onClick={resetTest} style={{ marginTop: '10px', padding: '10px 20px', background: '#3182ce', color: 'white', border: 'none', borderRadius: '5px' }}>Yenidən</button>
        </div>
      )}
      <div style={{ marginTop: '40px', background: '#fff', padding: '20px', borderRadius: '15px' }}>
        ...
        {liderler.map((l, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px', borderBottom: '1px solid #eee' }}>
            <span>{i+1}. {l.ad}</span>
            <span>{l.xal} {appMode === 'shooter' ? 'Xal' : 'wpm'}</span>
          </div>
        ))}
      </div>
    </div>
